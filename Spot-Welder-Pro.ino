...
void setup() {
  ...
  // Перевірити, чи натиснуто кнопку під час завантаження.
#ifdef _BOOTSYS_
  mEvent = EV_BOOTDN;
#else
  ...
  // Ініціалізація INA226 з повторними спробами
  {
    ...
    if (!inaOk) {
#ifdef _DEVELOPMENT_
      Serial.println(F("INA226 init failed after 3 retries! Rebooting..."));
#endif
      delay(1000);
      reboot();  // Перезавантаження замість зависання
    }
  }
